...
export class CircleDonutChartMapV2 {



	constructor() {
		this.map = undefined;
		this.circleLayerSource = `parkingLotWeMo-circle-source`;
		this.symbolLayerSource = `parkingLotWeMo-symbol-source`;
		this.mapConfig = {};
		this.markers = {};
	...
	setupDataSource(map, map_config, data) {
		...
		this.map.addSource(this.dataSourceId, {
			...
			clusterProperties: {
				...
				// 分成五類，每一類 count++
				mag1: ["+", ["case", ["<", ["get", "totalcar"], 500], 1, 0]],
				mag2: ["+", ["case", ["all", [">=", ["get", "totalcar"], 500], ["<", ["get", "totalcar"], 1000]], 1, 0]],
				mag3: ["+", ["case", ["all", [">=", ["get", "totalcar"], 1000], ["<", ["get", "totalcar"], 2000]], 1, 0]],
				mag4: ["+", ["case", ["all", [">=", ["get", "totalcar"], 2000], ["<", ["get", "totalcar"], 3000]], 1, 0]],
				mag5: ["+", ["case", [">=", ["get", "totalcar"], 3000], 1, 0]]
			}
		});
	}
